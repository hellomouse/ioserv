module.exports = function(bot) {
    bot.addCmd('kill', 'oper', event => {
        if (event.args[0]) {
            event.args[0].split(',').forEach(nick => {
                bot.whois(nick, (err, ident, whost, realname, ccnick, eid) => {
                    if (err) {
                        event.reply('No such nick');
                        return;
                    }
                    bot.kill(eid, event.args.slice(1).join(' '));
                });
            });
        } else {
    ...
    */
    ...
    bot.servmsg.on("SJOIN",(head,msg,serv) => {
        let users = msg.map(a=>a.replace("@","").replace("+","")).map(uid=>(bot.getUser(uid)||{nick:"unknown user"}).nick).join(", ");
